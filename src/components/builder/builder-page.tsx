...
import { Check, ChevronRight, Copy, History, MoreVertical, Pencil, Plus, Redo2, Square, Trash2, Undo2, Zap } from "lucide-react";
...
import { useDuplicateFlows, useTrashFlows, useUpdateFlows } from "@/lib/queries/automation";
import { useFlow, useFlowTriggers, usePublish, useSaveDraft, useSetFlowStatus, useVersions, type FlowDetail } from "@/lib/queries/builder";
...
function Editor({ flow, triggers }: { flow: FlowDetail; triggers: ReturnType<typeof useFlowTriggers>["data"] & object }) {
  const t = useT();
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <Toolbar flow={flow} saving={saving || save.isPending} dirty={draft !== lastSaved.current} onSaveNow={flush} onSetLive={setLive} publishing={publish.isPending} />
          ...
          {chain.map((node, i) => (
            <div key={node.id}>
              <section className="relative rounded-[24px] border border-border bg-bg p-6 shadow-sm">
                ...
                <MessageStepEditor node={node} readOnly={!canEdit} />
              </section>
              {i < chain.length - 1 && (
